...
def cmd_basic(args):
    ...
    if result.lex_errors:
        for e in result.lex_errors:
            print(
                f"[LEXER ERROR] {e['msg']} ({e.get('line', 0)}:{e.get('col', 0)})",
                file=sys.stderr,
            )
        sys.exit(1)
    if result.parse_errors:
        for e in result.parse_errors:
            print(
                f"[PARSER ERROR] {e['msg']} ({e.get('line', 0)}:{e.get('col', 0)})",
                file=sys.stderr,
            )
        sys.exit(1)
...
def cmd_compile(args):
    ...
    if ext == ".asm":
        ...
        if errs:
            for e in errs:
                print(
                    f"[ASM ERROR] {e['msg']} (line {e.get('line', 0)})", file=sys.stderr
                )
            sys.exit(1)
...
def cmd_run(args):
    ...
    if not rom_dir:
        rom_dir = "."
        print(
            f"[C64]    Warning: C64 ROMs not found. Copy ROM files to {rom_dir}",
            file=sys.stderr,
        )
    ...
    try:
        ...
        c64 = C64(enable_sid=args.sid, enable_resid=args.resid, vic_emulation="fast")
        ...
        print(
            "[C64]    c64py not installed. Install with: pip install c64py",
            file=sys.stderr,
        )
        print(
            "[C64]    PRG was generated anyway. Use VICE: x64sc " + prg_path,
            file=sys.stderr,
        )
...
def cmd_editor_tokenize(args):
    sys.path.insert(
        0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "editor")
    )
...
def main():
    ...
    p_compile.add_argument(
        "--basic-only", action="store_true", help="Genera solo BASIC"
    )
    ...
    p_run.add_argument(
        "--timeout", type=int, default=30, help="Timeout secondi (default 30)"
    )
    ...
    p_disk.add_argument(
        "--format", choices=["d64", "d81"], help="Formato disco (per create)"
    )
